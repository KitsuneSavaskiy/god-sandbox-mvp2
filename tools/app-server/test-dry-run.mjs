...
async function test48_eventStandingLaneGenerates8Prompts() {
  ...
  const writtenFiles = result.files.map((f) => f.replaceAll("\\", "/"));

  for (const expr of EVENT_EXPRESSIONS) {
    const exprFile = writtenFiles.find((f) => f.includes(`event-expressions/${expr}.prompt.md`));
    assert.ok(
      exprFile,
  ...
  const eventPromptFiles = writtenFiles.filter((f) => f.includes("event-expressions/"));
  assert.strictEqual(
