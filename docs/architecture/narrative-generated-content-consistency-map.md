...
6. 未生成、確認待ち、不採用、長すぎる文は `fallback` で進む。
...
| pack status | `candidate / needs-review / rejected / adopted / fallback` | 整合 |
| review gate | world canon / tone / safety / product / PO review を通す | 整合 |
| fallback | narrative未生成でも deterministic summary / result 文で進む | 整合 |
...
### 1. `ready` と `adopted` の表記差分

`docs/architecture/sandbox-generated-content-state-matrix.md` では narrative state に `ready` が残っている。
一方で `docs/architecture/narrative-pack-schema.md` と open PR群は `adopted` を採用状態として使っている。

短期判断:

- open PR #152 / #154 / #157 / #158 は `adopted` を使っているため、そのまま進めてよい。
- 既存state matrixは follow-up で、narrative状態を `adopted` に寄せるか、`ready` をUI表示名の alias として注記する。
...
- `candidate / needs-review / rejected / adopted / fallback` の扱いが分かる。
