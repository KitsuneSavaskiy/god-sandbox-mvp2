...
この文書は、生成された文章をそのまま正本にしないための schema、状態、review gate、fallback を定義する。
...
- `candidate`、`needs-review`、`rejected`、`adopted`、`fallback` の状態が定義されている。
...
schema の一部が未確定でも、`fallback` 状態を使い、ゲーム本体は既存 deterministic 文で進む。
