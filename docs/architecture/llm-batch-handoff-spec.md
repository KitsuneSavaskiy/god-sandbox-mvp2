...
**目的:** 箱庭世界状態を `DialogueWorldDigest` + `DialoguePromptPack` としてまとめ、外部 LLM（ChatGPT 等）への手動渡し用パケットを生成する。アプリ本体は外部 LLM API を直接呼ばない。

**候補ソースモードと境界:**

| ソースモード | LLM 使用 | 説明 |
|---|---|---|
| `authored_fixture` | 使わない | 人手で書いた発話 fixture。LLM 品質の最終判断には使えない |
| `external_llm_handoff` | 外部（手動） | PO または Codex Sidekick が外部 LLM へ手動で渡す |

**`external_llm_handoff` モードの制約:**
- `buildDialogueWorldDigest(session)` が `DialogueWorldDigest` を返す
- `buildDialoguePromptPack(digest)` が外部 LLM へ渡すフォーマット済みテキストを返す
- PO または Codex Sidekick が手動で外部 LLM へコピーし、発話候補テキストを受け取る
