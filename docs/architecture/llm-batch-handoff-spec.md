...
**目的:** 箱庭世界状態を `DialogueWorldDigest` としてまとめ、外部 LLM（ChatGPT 等）への手動コピー用パケットを生成する。アプリ本体は外部 LLM API を直接呼ばない。

制約:
- `buildDialogueWorldDigest(session)` が `DialogueWorldDigest` を返す
- ユーザー（PO）が手動で外部 LLM に `DialogueWorldDigest` JSON をコピーし、発話候補テキストを受け取る
- LLM が生成した発話候補は `DialogueCandidate.reviewStatus = "needs_review"` で保存する
- PO レビュー（`observed-dialogue-spec.md §9` の 7 点基準）後に `"accepted"` へ昇格させる
- `"needs_review"` 状態の候補を runtime で使用してはならない
...
- relation スコア（参照のみ。スコアを発話に埋め込んではならない）
- 直近イベントの自然言語要約（数値なし）
- world context tag
