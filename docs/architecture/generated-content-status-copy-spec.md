...
キャラクリエイト、住民一覧、CharacterDetailPanel で表示する Codex 生成状態、確認待ち、fallback 状態の文言を、非技術者にも分かる形で固定する。
...
- `ready` は manifest / read model 側で検査、人間確認、必要な PO 確認を通った状態として扱う。
...
現行 manifest / read model が持つ状態は `missing`、`placeholder`、`rejected`、`ready` を基本にする。`draft-ready`、`pending`、`running`、`needs-review` は、将来の job draft / generated content review layer が持つ補助状態として扱う。
...
| `needs-review` | 確認が必要 | 候補を ready として表示しない。 |
| `rejected` | 不採用 / 通常画像で代用中 | 不採用理由を詳細画面に短く出してよい。 |
| `ready` | 準備済み | 検査、人間確認、必要な PO 確認を通った状態。 |
...
壊れた asset や未確認 narrative は ready として扱わず、ユーザーには「通常画像で代用中」「標準のイベント文で進みます」と伝える。
