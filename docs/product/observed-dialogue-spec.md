...
§2.3-a（PBI 4a PO Preview）:

9. `buildDialogueWorldDigest(session)` が `activeCharacters` を1件以上含む `DialogueWorldDigest` を返す
10. `DialogueCandidate.reviewStatus` は `"needs_review"` で初期化される
11. `validateDialogue` で拒否された候補を `reviewStatus: "rejected"` に設定できる
12. `"needs_review"` 状態の候補は runtime（generateDialogue）で使用されない

---

## 9. PBI 4a: PO Preview — LLM 手動ハンドオフ用

この節は PBI 4a（Observed Dialogue PO Preview）の型と運用ルールを定義する。
PBI 4b（Runtime）の実装前提となる。

### 目的

箱庭の現在世界状態を `DialogueWorldDigest` にまとめ、PO が外部 LLM（ChatGPT 等）へ手動でコピーし発話候補を取得できるようにする。アプリ本体は外部 LLM API を直接呼ばない。
...
LLM が生成した発話候補を `"accepted"` に昇格させる前に、以下を確認する。
...
buildDialogueWorldDigest(session)
  ...
  → [PO が外部 LLM にコピー]
  → LLM 生成テキスト
  → DialogueCandidate { reviewStatus: "needs_review" }
  → PO レビュー（上記7点）
