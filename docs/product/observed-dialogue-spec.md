...
ソースモードは `authored_fixture`（人手 fixture）と `external_llm_handoff`（外部 LLM 手動渡し）の 2 通りを持つ。
アプリ本体は外部 LLM API を直接呼ばない。

### ソースモード

```ts
type DialogueCandidateSource =
  | "authored_fixture"       // 人手で書いた fixture 発話
  | "external_llm_handoff";  // 外部 LLM へ手動で渡して取得した候補
```

**`authored_fixture` モード**
- LLM を使わない
- 人間が書いた発話 fixture を使う
- UI 表示・吹き出し位置・発話頻度・Type A/B/C の境界・`doNotSay` フィルタ・`faithBand` 別の距離感を確認する
- LLM 生成品質・「うちの子らしさ」の最終判断には使えない

**`external_llm_handoff` モード**
- GodSandbox core app は LLM API を呼ばない
- `buildDialogueWorldDigest(session)` で `DialogueWorldDigest` を生成し、`buildDialoguePromptPack(digest)` でプロンプトパックを作る
- PO または Codex Sidekick が外部 LLM（ChatGPT 等）へ手動で渡す
