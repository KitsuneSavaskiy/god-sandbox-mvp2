...
type PassportCharacterProfile = {
  ...
  assetRef: {
    portraitUrl: string;          // ローカルファイルパスまたは URL
...
VoiceProfile の外部公開サブセット。内部制約ではなく AI への指示として整形する。
...
```json
{
  ...
  "character": {
    ...
    "assetRef": {
      "portraitUrl": "public/art/characters/defaults/garan/portrait.png"
    }
  },
  "lifeMemory": {
  ...
  "voiceProfile": {
    ...
    "doNotSay": ["ゲームUIの言葉（「セーブ」「スキル」）", "キャラクターの未設定設定の断言"],
