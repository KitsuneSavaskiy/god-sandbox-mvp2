...
| time-season copy | `#140` | Draft | sandbox time / season HUD copy 仕様 |
| asset sidekick | `#136` | Draft | asset sidekick personas 仕様 |
...
- Draft PR は、実装PRの表示や用語と矛盾しない状態まで仕上げてからReady for reviewにする。
...
1. `#152` で narrative surface の表示面を定義する。
2. `#154` で event UI acceptance matrix を定義する。
3. `#157` で mobile / accessibility QA を定義する。
4. `#158` で time / season narrative hook を接続する。
...
`#140` はDraftのcopy仕様PRである。
`#144` の実装表示と用語が矛盾しない状態にしてからReady for reviewにする。
...
`#136` はDraftのasset sidekick persona仕様PRである。
...
- `#152` / `#154` / `#157` / `#158` のnarrative surface chainが整理されている。
...
- narrative chain は `#152` から順に確認し直す。
...
- `src/**` の修正
- `#152` / `#154` / `#157` / `#158` / `#144` / `#140` / `#136` のbranch修正
