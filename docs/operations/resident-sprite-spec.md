...
2. Prompt path is recorded (auto-generated by `sidekick:intake` from `_template.md`).
3. Codex agent used Codex pet with the portrait and prompt to generate the sprite sheet.
...
## Generation source
- source portrait:
- prompt: (auto-generated by sidekick:intake / pre-existing)
