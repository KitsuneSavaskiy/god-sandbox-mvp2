...
ここでいう Codex Sidekick は、GodSandbox本体の中に入る機能ではなく、ローカル作業フォルダに置かれた制作依頼を読んで、asset候補やnarrative候補を作る将来の補助役である。
...
- 生成候補ができても、review gateを通るまでは `ready` / `adopted` ではない。
...
現時点では、次のPhase 1文書はまだmainに存在しない。
...
これらがmainへ入った後は、この文書のやさしい説明が、各詳細specの意味と矛盾しないか確認する。
...
| 種類 | 意味 |
| --- | --- |
| asset candidate | 画像やspriteなどの候補。まだ `ready` ではない。 |
| narrative candidate | 発話やイベント文の候補。まだ `adopted` ではない。 |
| `done` job | Sidekickが候補を返した状態。採用済みではない。 |
| `ready` asset | review gate後に使ってよいasset。 |
| `adopted` narrative | review gate後に使ってよい文章。 |

生成担当は、自分の生成物を単独で `ready` / `adopted` にしない。
...
- 生成候補を人間確認なしで `ready` / `adopted` にする流れになった。
...
Phase 2では、Line 1 / Line 2 / Line 4 の詳細specと合わせて、実装者が安全にrunner / bridge / test planへ進める。
