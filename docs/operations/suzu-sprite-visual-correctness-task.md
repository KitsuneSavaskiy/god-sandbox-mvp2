# Suzu resident sprite visual correctness task

Status: Sprint9 asset task document

## Purpose

This task defines the requirements for generating a correct Suzu resident sprite sheet.
...
When a Suzu resident sprite sheet is marked `ready`, the player sees a single, coherent, small sandbox character in the world.
...
For Suzu specifically, the final result is:

- Suzu is shown as a complete small resident sprite in `/sandbox`.
- Eve / Garan / Ryo continue unaffected.
...
The generation prompt must be based on `.prompts/resident-sprites/suzu.md`, not ad-hoc chat instructions.
...
- same visual identity as `public/art/characters/defaults/suzu/portrait.png`,
...
node tools/asset-pipeline/process-resident-sprite-sheet.mjs suzu
...
```bash
codex "Read docs/operations/suzu-sprite-visual-correctness-task.md, implement the task exactly, achieve the final vision, and test until complete."
```
