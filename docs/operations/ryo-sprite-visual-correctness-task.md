# Ryo resident sprite visual correctness task

Status: Sprint9 asset task document

## Purpose

This task defines the requirements for generating a correct Ryo resident sprite sheet.
...
When a Ryo resident sprite sheet is marked `ready`, the player sees a single, coherent, small sandbox character in the world.
...
For Ryo specifically, the final result is:

- Ryo is shown as a complete small resident sprite in `/sandbox`.
- Eve / Garan / Suzu continue unaffected.
...
The generation prompt must be based on `.prompts/resident-sprites/ryo.md`, not ad-hoc chat instructions.
...
- same visual identity as `public/art/characters/defaults/ryo/portrait.png`,
...
```bash
codex "Read docs/operations/ryo-sprite-visual-correctness-task.md, implement the task exactly, achieve the final vision, and test until complete."
```
