...
| proposedPath | `public/art/events/{templateId}/{phase}.png` |
...
実画像は現時点で未追加。UI 実装時の安全弁として path を予約する。
...
| 状態 | 内容 |
|---|---|
| 未制作 | 全 7 event の started 画像、fallback 画像 |
| 制作依頼の手順 | `docs/art-prompts/event-images/preauthored-event-art-briefs.md` を参照 |
| 採用フロー | PO 確認後に別 PBI で `public/art/events/` へ配置 |
